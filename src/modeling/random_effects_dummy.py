...
from src.modeling.fixed_effects import add_dummy_for_date
...
def dummy(
    file_path: str,
    X_var_names=["PR", "CDR", "LAS", "CR", "NIA", "OE", "RA", "ROA", "SCTA", "SIZE", "is_END19", "is_END20", "is_END21", "is_END22", "is_END23", "is_WAR"],
    dummy_dates=[],
    dummy_names=[],
    show_all=False,
    aver=0,
    ...
    #     index_col=0,
    # )
    df_panel = add_dummy_for_date(file_path, dummy_dates, dummy_names)
    df_panel = df_panel.dropna()
    for bank in bank_names:
        df_panel.drop('is_' + bank, axis=1, inplace=True)
